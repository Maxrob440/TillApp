...
namespace TillApp
{
    class DataBaseHelper
    {
        ...
        public void addTransaction(List<Object> Transaction)
        {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                {
                    SQLiteCommand get_latest_id = new SQLiteCommand("SELECT MAX(ORDER_ID) FROM ORDERS", connection);
                    ...
                    foreach (List<int> item in Transaction)
                    {
                        ...
                        SQLiteCommand add_command = new SQLiteCommand("INSERT INTO ORDERS (ORDER_ID ,ITEM_ID,QUANTITY) VALUES (@TransactionID,@itemID,@quant)", connection);
        ...
        public String get_name_from_itemID(int itemID) {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                connection.Open();
                SQLiteCommand get_name_from_itemID = new SQLiteCommand("SELECT ITEM_NAME FROM ITEMS WHERE ITEM_ID =@itemID", connection);
                get_name_from_itemID.Parameters.AddWithValue("@itemID", itemID);
                SQLiteDataReader reader = get_name_from_itemID.ExecuteReader();
        ...
        public float get_price_from_itemID(int itemID)
        {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                connection.Open();
                SQLiteCommand get_name_from_itemID = new SQLiteCommand("SELECT PRICE FROM ITEMS WHERE ITEM_ID =@itemID", connection);
                get_name_from_itemID.Parameters.AddWithValue("@itemID", itemID);
                SQLiteDataReader reader = get_name_from_itemID.ExecuteReader();

                if (reader.Read())
                {
                    float itemPrice = Convert.ToSingle(reader["PRICE"]);
                    return itemPrice;
                }
            }
            return (float)0.0;
